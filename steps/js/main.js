jQuery(document).ready(function($) {
	//store step items
	var fillingBlocks = $('.cd-step').not('.cd-step-divider');

	//store step items top position into an array
	...
	//add the .focus class to the first step item
	fillingBlocks.eq(0).addClass('focus');

	$(window).on('scroll', function(){
		//check which step item is in the viewport and add the .focus class to it
...
function bodyBackground(itemsTopValues) {
	var topPosition = $(window).scrollTop() + $(window).height()/2,
		stepsNumber = itemsTopValues.length;
	$.each(itemsTopValues, function(key, value){
		if ( (itemsTopValues[key] <= topPosition && itemsTopValues[key+1] > topPosition) || (itemsTopValues[key] <= topPosition && key+1 == stepsNumber ) ) {	
